...
def email_inbox(request):
    ...
    res = service.users().messages().list(
        userId="me", labelIds=["INBOX"], q=q or None, pageToken=page or None, maxResults=limit
    ).execute()
    ...
    for msg in res.get("messages", []) or []:
        ...
        email_data.append({
            "id": meta["id"],
            "em_subject": meta["subject"],
            "em_to": meta["from"],
            "em_message": meta["snippet"],
        })
...
INTENT_TEMPLATES: Dict[str, Dict[str, str]] = {
    ...
    "cancel":   {"subject": "Subscription Cancellation Update",
                 "body": "Hi,\n\nWe are sorry to see you go. Your subscription has been scheduled for cancellation. "
                         "If this was a mistake, reply to this email and we'll help you reverse it.\n\nRegards,\nSupport Team"},
    "refund":   {"subject": "Refund Request Received",
                 "body": "Hi,\n\nWe have noted your refund request. Please share your order ID so we can process it. "
                         "Refunds are available within 7 days of purchase per our policy.\n\nRegards,\nSupport Team"},
    "support":  {"subject": "We are On It — Support Ticket Opened",
